...
RUN mkdir /code && apt-get update && apt-get install -y python-pip
...
USER postgres
RUN /etc/init.d/postgresql start && sleep 120 &&\
    createuser gisuser &&\
...
RUN /etc/init.d/postgresql restart && sleep 120 && mongod --fork --logpath /var/log/mongodb.log \
    && ./create_tables.sh \
